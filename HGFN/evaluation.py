...
def evaltop(model_path, data_path=None, split='dev', fold5=False):
    ...
    if not fold5:
        ...
        topk = top(img_embs, sims, return_ranks=True)
    return topk
...
def shard_xattn(model, images, captions, bbox, caplens, opt, shard_size=128):
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            print(len(im),len(s),len(l),len(bbx))
            # 修复小批量问题：如果im和bbx长度只有1，就自我复制
            if len(im) == 1:
                im = torch.cat([im, im], dim=0)  # 复制一次，长度变为2
                bbx = torch.cat([bbx, bbx], dim=0)  # 同样复制边界框
                # 注意：这里我们复制了输入，但只需要第一个结果
            
            # 计算相似度
            sim = model.forward_sim(im, s, bbx, l)
            
            # 如果之前复制了，只取第一个结果
            if im_start == im_end - 1:  # 如果原来只有1个图像
                sim_result = sim[0:1].data.cpu().numpy()  # 只取第一个
            else:
                sim_result = sim.data.cpu().numpy()
                
            d[im_start:im_end, cap_start:cap_end] = sim_result
...
def top(images, sims, npts=None, return_ranks=False):
    """
    Images->Text (Image Annotation)
    Images: (N, n_region, d) matrix of images
    Captions: (5N, max_n_word, d) matrix of captions
